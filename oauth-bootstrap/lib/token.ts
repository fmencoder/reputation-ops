/**
 * Token normalisation and safe fingerprinting — bootstrap copy.
 *
 * GENERATED FROM src/novra/token.ts. Do not edit by hand.
 *
 * The bootstrap deploys as its own project and cannot import from the
 * repository root, so this is a copy rather than a shared module. It is not
 * allowed to drift: test/novra/token.test.ts runs both implementations over the
 * same corpus and fails if any input produces a different normalisation,
 * fingerprint or rejection. A fingerprint comparison between two systems is
 * only evidence if both compute it the same way.
 */
