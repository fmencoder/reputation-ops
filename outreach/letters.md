# Correspondence — ready to send

Send from the subject's own address. No letterhead, no law-firm styling, no
reference to counsel — no counsel is involved.

Status on all three: **READY / SEND_BLOCKED_BY_TOOLING**. This session has no
email or form-submission capability.

---

## 1. MoreLaw — Notice of factual inaccuracy and request for remedial editorial action

**To:** verdicts@morelaw.com
**Cc:** info@morelaw.com
**Attn:** Kent Morlan, Esq., Editor & Publisher
**Subject:** Request for correction and remedial editorial action — United States of America v. Fredrick Mendez, MoreLaw case CO/188850

**Status: SEND-READY.** Predicate verified by the subject from the current live
page. One slot remains: `[EXACT_URL]`, the case page address — paste it from
your browser bar. The case number and caption below are already exact.

**Pre-send check, thirty seconds:** reload the page and confirm the Outcome and
FAQ still read "found guilty". If MoreLaw has already amended it, send the
consolidation requests (items 2–6) without item 1. Sending a correction notice
against wording that has since changed invites the whole letter to be dismissed.

---

> Dear Mr. Morlan,
>
> I am Fredrick Mendez, the defendant named in *United States of America v.
> Fredrick Mendez*, published by MoreLaw as case CO/188850 at [EXACT_URL].
>
> This correspondence is intended to provide precise notice of an inaccuracy in
> that entry and of the remedial actions requested. Please treat it as a formal
> request for editorial review.
>
> **The inaccuracy is procedural, and it is specific.**
>
> The entry's Outcome field states that the defendant was "found guilty", and the
> FAQ section repeats that formulation. That is not what happened. The matter was
> resolved by guilty plea, not by verdict after trial.
>
> The authoritative source is the announcement issued by the U.S. Attorney's
> Office for the District of Colorado in January 2025, which states that I was
> sentenced "after pleading guilty to wire fraud":
>
> https://www.justice.gov/usao-co/pr/florida-man-sentenced-41-months-stealing-covid-19-relief-funds
>
> I want to be precise about the scope of this request. **Nothing here disputes
> the underlying public record.** The conviction, the 41-month sentence, the
> three-year term of supervised release and the restitution order of
> $1,589,565.75 are accurate and are not in dispute. I am not asserting a claim
> of defamation, I am not asserting any statutory claim, and I make no
> representation about the legal consequences of the distinction.
>
> What I am asking is that a legal-research publication describe the disposition
> of a case correctly. "Found guilty" and "pleaded guilty" denote different
> procedural events, and MoreLaw's readership is precisely the audience for which
> that difference carries meaning. A publication of case outcomes has its own
> reason to want the outcome field right.
>
> **Requested remedial actions:**
>
> **1. Correction.** Amend the Outcome field and the FAQ formulation in case
> CO/188850 to state accurately that the defendant pleaded guilty to wire fraud —
> for example, "pleaded guilty to wire fraud" or "sentenced after pleading guilty
> to wire fraud".
>
> **2. Duplicate consolidation.** This case material appears across multiple
> MoreLaw entries and Colorado practice-area index pages. Please remove the
> redundant duplicate entries.
>
> **3. Category and index removal.** Please remove this entry from practice-area
> and category index pages where the case is not substantively relevant to the
> category, and remove my name from index snippets and headings where editorially
> feasible.
>
> **4. Canonicalisation.** Please consolidate the remaining case material behind a
> single canonical URL.
>
> **5. Noindex fallback.** Where a duplicate, archive or category surface is
> retained, please apply a noindex directive to it rather than leaving multiple
> indexed copies of one case summary.
>
> **6. Written confirmation.** Please confirm in writing which URLs have been
> corrected, removed, consolidated, canonicalised or excluded from indexing,
> identifying each one.
>
> If item 1 is the only action you are prepared to take, I would still be
> grateful for it. If you are not prepared to act on item 1, please evaluate
> items 2 through 5 independently rather than treating this as a single
> all-or-nothing request.
>
> On the duplication point specifically: reproducing one case summary across
> several category and practice-area surfaces is not necessary to preserve public
> access to the record. The Department of Justice announcement remains available
> in full and is unaffected by anything MoreLaw decides here.
>
> I would be grateful for a response by [DATE +10 DAYS]. I will follow up once,
> and I will respect your decision either way.
>
> Respectfully,
>
> Fredrick Mendez
> [contact email]
> [date]

---

## 2. Hoodline — editorial discretion request

**To:** Hoodline editorial contact — verify the current address before sending
**Subject:** Formal request for editorial review — January 2025 article

> To the Editorial Team,
>
> I am Fredrick Mendez, the subject of the article published on 2 January 2025
> at:
> https://hoodline.com/2025/01/boca-raton-man-sentenced-to-over-3-years-for-fraudulently-obtaining-covid-19-relief-funds/
>
> This correspondence is intended to provide precise notice of the affected URL
> and the remedial actions requested. Nothing in this request disputes the
> underlying public record. The article reports it accurately, and I am making
> no legal claim of any kind.
>
> The basis for the request is editorial rather than factual. The piece is a
> summary of a Department of Justice announcement rather than original local
> reporting, and that announcement remains publicly available in full. Public
> access to the record is therefore preserved regardless of what Hoodline
> decides. What the article adds is a second, name-indexed surface, and it is
> that additional amplification — rather than the existence of the record — I am
> asking you to weigh.
>
> **Requested actions, in order of preference:**
>
> 1. Removal of the article.
> 2. Replacement of my name with a general description ("a Boca Raton man"),
>    which the headline already uses.
> 3. Removal of my name from article metadata.
> 4. Removal of the article from category, tag and author index surfaces.
> 5. Application of a noindex directive to the article.
> 6. Removal from internal search and recommendation surfaces.
>
> I note the headline and URL slug already carry no personal name, so items
> relating to those are moot.
>
> If complete removal is not available, please evaluate the narrower measures
> independently rather than treating this as a single all-or-nothing request.
>
> If first-line support is not able to act on this, I would be grateful if you
> would escalate it to editorial leadership for a decision. Please confirm that
> decision in writing, whichever way it goes, and identify any URLs modified.
>
> I would appreciate a response by [DATE +10 DAYS]. I will follow up once and
> will respect the outcome either way.
>
> Respectfully,
> Fredrick Mendez
> [contact address]

---

## 3. Bloomberg Law — archival review and canonicalisation

**To:** Bloomberg Law editorial contact — verify the current address
**Subject:** Editorial and archival review request — duplicate URL paths

> Dear Editor,
>
> I am Fredrick Mendez, the subject of "Florida Man Sentenced to Prison for
> Stealing Covid Relief Funds".
>
> I want to be precise about what I am and am not raising. **I am not asserting
> any factual inaccuracy.** The article correctly states that I pleaded guilty.
> I am making no legal claim, and I understand that Bloomberg Law is a
> publication of record whose archive exists for good reason. I have kept this
> request correspondingly narrow.
>
> **Primary request — duplicate URL paths.** The article appears to be served
> under two paths:
>
> - /us-law-week/florida-man-sentenced-to-prison-for-stealing-covid-relief-funds
> - /white-collar-and-criminal-law/florida-man-sentenced-to-prison-for-stealing-covid-relief-funds
>
> If these are the same piece, consolidating them behind a single canonical URL
> would be ordinary archival housekeeping, independent of anything about me. I
> would be grateful if you would consider it.
>
> **Secondary requests, entirely at your discretion:**
>
> 1. Archival and editorial review of the piece given its age and its nature as
>    a brief summary of a government announcement.
> 2. Name minimisation in the body text.
> 3. A noindex directive.
> 4. Removal from secondary category and discovery surfaces.
> 5. Removal of any redundant indexed versions.
>
> Please treat this as a formal request for editorial review. I would be
> grateful if you would confirm in writing which URLs, if any, have been
> modified, canonicalised or excluded from indexing.
>
> I understand the answer to most of the above may be no, and I will respect
> that. I would appreciate a response by [DATE +10 DAYS] and will follow up once.
>
> Respectfully,
> Fredrick Mendez
> [contact address]

---

## Follow-up and escalation

**NO_RESPONSE** — one follow-up at day 10, three sentences, restating the URL
and the narrowest request only. Never a second follow-up.

**PARTIAL** — accept it. Record exactly which URLs changed and how. That change
is what arms the search-engine refresh path; nothing else does.

**REJECTED** — stop. Do not re-approach on the same ground. Assess only whether
a genuinely different and legitimate route exists; if none does, the target
moves to suppress-and-monitor. Repeated contact after a clear decision is
harassment and it would poison the other two requests.

**NEEDS_INFO** — supply only what is verified. Never fill a gap with an
assumption to keep a request moving.
